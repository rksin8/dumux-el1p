...
public:
    ...
    , couplingManagerPtr_(couplingManagerPtr)
    {
        ...
        loadForce_  =  getParam<Scalar>("PoroElastic.Problem.F");
    }
    ...
    { 
        ...
        if (globalPos[1] > 20.0 - eps_) // Top
            values[uyIdx] = -loadForce_; // Fy downward compressive
    ...
    BoundaryTypes boundaryTypesAtPos(const GlobalPosition& globalPos) const
    {
        ...
        else if (globalPos[1] < 0.0 + eps_){ // Bottom
            values.setAllDirichlet();
        }
        else if (globalPos[0] < 0.0 + eps_ 
                || globalPos[0] > 10.0 - eps_ ){ // Left and Right
            values.setDirichlet(uxIdx);
        }
